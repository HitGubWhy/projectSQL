...
const registerHtml = fs.readFileSync(path.join(__dirname, "static", "register.html"))
const authFile = fs.readFileSync(path.join(__dirname, "static", "auth.js"))
const loginHtml = fs.readFileSync(path.join(__dirname, "static", "login.html"))
...
function registerUser(req, res) {
    ...
    req.on('end', async function() {
        try {
            ...
            if (await db.idUserExist(user.login)) {
                return res.end("User already exist")
            }
            await db.addUser(user)
            return res.end("Registration successful")
        } catch(e) {
            console.log(e)
        }
    })
